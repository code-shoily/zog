  ...
  * `:hash_graph` - Hash Map Graph (`GraphMap`).
  ...
  if Code.ensure_loaded?(Zig) do
    ...
    pub fn nif_node_degrees(res: GraphRes) ![]u32 {
        ...
        switch (res.unpack()) {
            .soa => |g| {
                const first_edges = g.nodes.items(.first_edge);
                const to_slice = g.edges.items(.to);
                const next_edge_slice = g.edges.items(.next_edge);
                const edge_is_deleted = g.edges.items(.is_deleted);
                const node_is_deleted = g.nodes.items(.is_deleted);

                for (0..node_count) |i| {
                    var count: u32 = 0;
                    var edge_idx = first_edges[i];
                    while (edge_idx) |idx| {
            ...
            .hash_graph => |g| {
                for (0..node_count) |i| {
                    var count: u32 = 0;
                    if (g.out_edges.get(@intCast(i))) |list| {
    ...
    fn addParsedEdges(graph: anytype, edges: []ParsedEdge, is_directed: bool, arena: std.mem.Allocator) !void {
        _ = arena;
        if (is_directed) {
            for (edges) |edge| {
        ...
        const SortCtx = struct {
            fn lessThan(_: @This(), a: ParsedEdge, b: ParsedEdge) bool {
                const a_lo = @min(a.from, a.to);
                const a_hi = @max(a.from, a.to);
                const b_lo = @min(b.from, b.to);
                const b_hi = @max(b.from, b.to);
                if (a_lo != b_lo) return a_lo < b_lo;
                return a_hi < b_hi;
            }
        };
        std.mem.sort(ParsedEdge, edges, SortCtx{}, SortCtx.lessThan);

        var prev_lo: u32 = std.math.maxInt(u32);
        var prev_hi: u32 = std.math.maxInt(u32);
        for (edges) |edge| {
            const lo = @min(edge.from, edge.to);
            const hi = @max(edge.from, edge.to);
            if (lo == prev_lo and hi == prev_hi) continue;
